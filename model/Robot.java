...
public class Robot {
    ...
    public boolean stepFCD() {
        if (!this.started) {
            ...
            for (i = 0; i < mainDir.length &&
                ...
                ;
            if (i == mainDir.length) {
                Sensor back = sensorAt(pos).getPreviousSensor();
                if (back == null) {
                    this.fcdEnded = true;
                    return true;
                }
                move(back.getCoordinates());
            } else {
    ...
    Sensor sensorAt(Vec pos) {
        for (Sensor s : sensors)
            if (s.getCoordinates().equals(pos)) return s;
        return null;
    }

    boolean isCovered(Vec pos) {
        for (Sensor s : sensors)
            if (pos.dist(s.getCoordinates()) <= Values.sensorSensing)
                return true;
        return false;
    }
